...
public class UpdateEtude implements ComponentListener {
	...
	public void updateEtude(TabGraphe tab, int type)
	{
		...
		case 1:
			...
			try {FenetreAccueil.etude.getS1().setRis_contr_tech(((ConteneurSchema) tab.getConteneur_risque()).getContr_tech());} catch(NullPointerException ex) {}
			try {FenetreAccueil.etude.getS1().setRis_etabl_prot(((ConteneurSchema) tab.getConteneur_risque()).getEtabl_prot());} catch(NullPointerException ex) {}
			try {FenetreAccueil.etude.getS1().setRis_mena_mar(((ConteneurSchema) tab.getConteneur_risque()).getMena_mar());} catch(NullPointerException ex) {}
			try {FenetreAccueil.etude.getS1().setRis_press_envir(((ConteneurSchema) tab.getConteneur_risque()).getPress_envir());} catch(NullPointerException ex) {}
